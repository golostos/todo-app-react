...
export const login = createAsyncThunk('user/login', async function (user: userInfoDTO, { rejectWithValue }) {
    const response = await post('/api/user/login', user)
    if (response.ok) {
        const responseToken: LoginResponse = await response.json();
        return responseToken.token
    }
    else return rejectWithValue('Incorrect name or password')
})
...
export const { setUserError } = usersSlice.actions
